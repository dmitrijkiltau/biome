...
pub struct NeosFusionFileHandler;

lazy_static! {
    // https://regex101.com/r/E4n4hh/3
    pub static ref NEOSFUSION_FENCE: Regex = Regex::new(
        r#"(?ixms)(?:<script[^>]?)
            (?:
            (?:(lang)\s*=\s*['"](?P<lang>[^'"]*)['"])
            |
...
impl NeosFusionFileHandler {
    /// It extracts the JavaScript/TypeScript code contained in the script block of a Neos Fusion file
    ///
    ...
    /// It takes the original content of a Neos Fusion file, and new output of an Neos Fusion file. The output is only the content contained inside the
    /// Neos Fusion `<script>` tag. The function replaces `output` inside that `<script>`.
    ...
    /// Returns the start byte offset of the Neos Fusion `<script>` tag
    ...
    pub fn file_source(text: &str) -> JsFileSource {
        let matches = NEOSFUSION_FENCE.captures(text);
        matches
...
impl ExtensionHandler for NeosFusionFileHandler {
